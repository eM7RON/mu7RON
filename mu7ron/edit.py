...
def finalize_midi_sequence(ptrn, name='Track 1', inst=0, channel=0, tsig=(4, 4), bpm=120):
    '''
...
def filter_ptrn_of_evnt_typs(old_ptrn, typs_2_keep=tuple(), f=copy.deepcopy):
    ...
    for old_trck in old_ptrn:
        new_trck = midi.Track(
                              tick_relative = False#old_trck.tick_relative
                               )
...
def filter_ptrn_of_insts(old_ptrn, condition=lambda x: x.data[0] == 47 or x.data[0] > 95):
    '''
...
def filter_ptrn_of_percussion(x, typs=(midi.NoteOnEvent, midi.NoteOffEvent), channel=9, res=480):
    '''
...
def filter_data_of_empty_ptrn(data):
    ...
    '''
    return [ptrn for ptrn in data if analyze.has_evnt_typ(ptrn, (midi.NoteOnEvent, midi.NoteOffEvent))]
...
def time_split(x):
    ...
    '''
    if isinstance(x, midi.Pattern) or any([isinstance(trck, (list, midi.Track)) for trck in x]):
        x = [
            trck for trck in x if utils.safe_len(trck) and isinstance(trck, (list, midi.Track))
            ]
...
def _default_dedupe_criteria(e0, e1):
    ...
    '''
    if isinstance(e0, (midi.NoteOnEvent, midi.NoteOffEvent)) \
    and any([e.channel == 9 for e in [e0, e1]]):
...
def dedupe(x, **kwargs):
    ...
    if isinstance(x, midi.Pattern):
        assert len(x) == 1, 'Pattern must be uni-tracked, try using edit.consolidate_trcks beforehand'
        old_ptrn = x
        old_trck = x[0]
        old_ptrn.make_ticks_abs()
        mode = 'ptrn'
...
def replace_evnt_typ(old_ptrn, old_typ, new_typ, condition=lambda x: True, copy_func=None):
    '''
...
def apply_on_pulse(x, div, apply_func, gen_func):
    ...
    for i in range(len(chgs) - 1):
        ...
        while tick <= chgs[i + 1][0]:
            ret    = apply_func(**dict(alist=ret, tick=tick, on=on, res=x.resolution, tsig=tsig))
            tick += tpp
            on     = not on
    x.make_ticks_rel()
    return ret
