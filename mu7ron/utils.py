...
def flatten(alist, depth=0):
    ...
    if depth is 1:
        ...
        for item in alist:
            if isinstance(item, (list, tuple, set, np.ndarray)) and not isinstance(
                item, (str, bytes)
            ):
...
def ticks_per_bar_division(res, **kwargs):
    ...
    div = kwargs.get("div", 1.0)
    tsig = tsig[0] / (utils.nptf(tsig[1]) if raw else tsig[1])
    return round(res * tsig * 4.0 / div)
...
class MidiObj:
    ...
    def __init__(self, path=None, ptrn=None, sn=None):
        ...
        assert any(
            [has_path, has_ptrn]
        ), "MidiObj requires midi.Pattern or path to a saved midi.Pattern as input to __init__"
        ...
        if has_ptrn and not has_path:
            assert (
                sn is not None
            ), "A midi.ptrn without a path requires a serial number (sn) argument to act as an identifier and generate a unique path"
    ...
    def _init(self):
        ...
        self.n_tsig, self.tsig_data = analyze.get_evnt_info(
            self.ptrn, midi.TimeSignatureEvent
        )
        self.n_tmpo, self.tmpo_data = analyze.get_evnt_info(
            self.ptrn, midi.SetTempoEvent, ["tick", "get_bpm"], lambda x: [x[0], x[1]()]
        )
        self.inst_data = analyze.get_evnt_info(
            self.ptrn, midi.ProgramChangeEvent, ["data"]
        )[1]
    ...
    def __repr__(self):
        ...
        tsig_data.extend(
            [
                f"{x[0]}{' ' * (10 - len(str(x[0])))}| {translate_tsig(x[1])}"
                for x in self.tsig_data
            ]
        )
        ...
        tmpo_data.extend(
            [
                f"{x[0]}{' ' * (10 - len(str(x[0])))}| {round(x[1], 3)}"
                for x in self.tmpo_data
            ]
        )
