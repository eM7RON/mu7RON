...
def initialize_weights(x, n_vocab=146, eps=None):
    ...
    eps = 0.0 if eps is None else np.finfo("float32").eps
    ret += eps
...
def vocab_wts(
    x: list, n_time: int, n_vocab: int, off_mode: bool, rho: float = 0.4
) -> np.ndarray:
    """
    ...
    avnts = list(
        utils.flatten(aug.exhaustive_transpose(x, n_time=n_time, off_mode=off_mode))
    )
    cnt = utils.counter(avnts)
    n = len(avnts)
    for i in range(n_vocab):
        if i in cnt:
            ret[i] = cnt[i] / n
    np.clip(ret, np.finfo(ret.dtype).eps, 1.0)
    # ret /= np.max(ret)
    ret **= rho
    # ret /= np.max(ret)
    ret = 1.0 - ret
...
def semantic_wts(
    x: list, n_time: int, n_vocab: int, map_: dict, off_mode: bool, rho=0.5
) -> np.ndarray:
    """
