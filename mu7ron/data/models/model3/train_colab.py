...
print("retrieving all filenames ending '.mid'...")
fnames = []
...
data = [x for x in tqdm(data) if analyze.max_simultaneous_notes(x) < 7]
print(f'{len(data)} samples remaining...')
...
for trck in tqdm(data):
    try:
        categorized = translate.categorize_input(trck, 
                                 q=q,
                                 q_map=q_map,
                                 time_encoder=time_encoder,
                                 ekwa=ekwa,
...
print('saving splits...')
with open(os.path.join('gdrive', 'My Drive', 'backups', 'params_train_valid.pkl'), 'wb') as file:
...
autosaver = CustomCheckpoint(save_dir=os.path.join('gdrive', 'My Drive', 'backups'))
progress.update(1)
