...
def load_midi_map():
    ...
    for fn in fnames:
        with open(os.path.join(os.path.split(__file__)[0], 'data', 'maps', fn + '.json'), 'r') as file:
...
def augmentation_map(data, n_time, off_mode, lo_lim=40, hi_lim=100, 
                     t_range=[0.95, 1., 1.05]):
    '''
        ...
        if lo < hi:
            for p_aug in range(lo, hi):
                for t_aug in t_range:
                    ret.append({'orig_sample_idx':i, 'pitch_aug': p_aug, 'time_aug': t_aug})
    return ret

def teacher_forcing_map(data, aug_map, n_time, n_vocab,
                        n_input, n_output, n_sample, n_teach, n_step, buffer):
    '''
    ...
    for map_ in aug_map:
        ...
        for k in teach_range:
            ...
            new_map_['id'] = (new_map_['orig_sample_idx'], map_['pitch_aug'], map_['time_aug'])
...
def stateless_balanced_sampling_map(data, n_input, n_output, n_sample, n_example, n_vocab):
    '''
    ...
    for i, x in enumerate(data):
        ...
        for j in range(n_vocab):
            ...
            if n_choice:
                ...
                for k in idx:
                    ret.append({'orig_sample_idx':i, 'strt_idx': k - n_req + 1, 'end_idx': k + 1})
    return ret
