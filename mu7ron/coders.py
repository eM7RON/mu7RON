...
def process_timestep(master_sequence, timestep, tick, bpm, mspt, q, n_time, 
                      n_t_p, n_t_p_v, q_map, p_map, time_encoder, sort_velocity, sort_pitch, sort_pulse):
    '''
    ...
    for evnt in timestep:
        if isinstance(evnt, midi.SetTempoEvent):
            ...
            if isinstance(evnt, midi.NoteOffEvent):
                pitch += 128
            v_map[n_t_p + q_map['encode'][utils.quantize(evnt.data[1], q)]].append(pitch)
...
def categorize_input(x, q, n_time, off_mode, time_encoder, ekwa, **kwargs):
    ...
    static_args   = (q, n_time, n_t_p, n_t_p_v, q_map, p_map, t_encoder, sort_velocity, sort_pitch, sort_pulse)
...
def beam_search(sequence, n_beam, model=None, path_to_model=None, custom_objs=None, n_iter=3):
    '''
    ...
    assert n_beam < n_vocab, f'n_beam is not smaller than n_vocab i.e. {n_beam} not smaller than {n_vocab}'
    ...
    for i in tqdm(range(1, n_iter)):
        ...
        main_beam = [[temp_beam[j][0], np.vstack([main_beam[temp_beam[j][2]][1].copy(), temp_beam[j][1]])] for j in b_range]
...
def process_timestep_with_drums(
    master_sequence, timestep, tick, bpm, mspt, q, n_time, n_pitch, n_drum, n_note, n_t_n, n_t_n_v, 
    p_map, q_map, time_encoder, sort_velocity, sort_pitch, sort_pulse):
    '''
    An auxiliary function for 'categorize_input'. Once all events, which occur simultaneously, at 
    a particular timestep have been collected by encode_input, 'process_timestep' will process
    these events, encoding them as a sequence of integers. The master function 'encode_input' 
...
def categorize_input_with_drums(x, q, n_time, off_mode, drm_mode, time_encoder, ekwa, **kwargs):
    '''
    This function is the inverse of categorize_output.
    This function will encode a midi sequence in the form of a midi.Track/midi.Pattern
    (with 1 track) or a MuGen.utils.MidiObj into a encoded/serialized sequence of integers. 
    ...
    t_encoder     = lambda x: time_encoder(x, **ekwa)
    args          = [ret, timestep, tick, bpm, mspt]
    static_args   = (q, n_time, n_pitch, n_drum, n_note, n_t_n, n_t_n_v,
                     p_map, q_map, t_encoder, sort_velocity, sort_pitch, sort_pulse)
...
def decategorize_output_with_drums(x, q, n_time, off_mode, drm_mode, time_decoder, dkwa, **kwargs):
    '''
    This function is the inverse of categorize_input . 
    Decodes an iterable containing avnts into a midi.Pattern containing midi.events,
    'evnts'.
    ...
    for avnt in x:
        if isvelocity(avnt):
            ...
            if ison(avnt):
                evnt = midi.NoteOnEvent(tick=tick, data=[avnt - adj_on, velocity], channel=channel)
            else:
                evnt = midi.NoteOffEvent(tick=tick, data=[avnt - adj_off, velocity], channel=channel)
            trck.append(evnt)
            velocity_flag = False
