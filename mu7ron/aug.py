...
def random_choice_transpose(
    x,
    options=np.arange(-4, 5),
    wts=None,
    n_time=64,
    verbose=False,
    inplace=False,
...
def random_bounded_transpose(
    x, lo_lim=40, hi_lim=100, n_time=64, verbose=False, inplace=False, off_mode=False
):
    """
    Shifts all of the pitches values of the note avents in an encoded midi pattern by a
...
def bounded_exhaustive_transpose(
    x, lo_lim=40, hi_lim=100, n_time=64, verbose=False, off_mode=False
):
    """
    Shifts all of the pitches values of the note avents in an encoded midi pattern by a
    random amount. The shift will occur within the bounds of feasibile pitch values 
...
def permute_simultaneous_evnts(
    x: np.ndarray,
    n_time: int = 64,
    n_t_p: int = 253,
    inplace: bool = False,
...
def time_warp(
    ...
    asarray=True,
    dtype="int32",
    time_encoder=temporal.base_digits_encoder,
    ekwa=dict(b=64),
    time_decoder=temporal.base_digits_decoder,
    dkwa=dict(b=64),
    **kwargs,
):
    """
...
def random_choice_time_warp(
    x,
    n_req=None,
    options=np.linspace(0.95, 1.05, num=10),
    wts=None,
    n_time=125,
    asarray=True,
    dtype="int32",
...
class RandomDataAugGen(Sequence):
    def __init__(self, data, **kwargs):
        ...
        self.rtrn_mode = (
            True if version.parse(__version__) <= version.parse("2.1.0") else False
        )
        ...
        self.rtrn_map = {True: lambda x, y: (x, y), False: lambda x, y: (x, y, [None])}
    ...
    def __getitem__(self, idx):
        ...
        for j in self.batch_loop:
            ...
            x[j, self.in_range, sub[: self.n_input]] = 1.0
            y[j, sub[self.n_input : self.n_io]] = 1.0
...
class MappedDataAugGen(Sequence):
    def __init__(self, data, **kwargs):
        ...
        self.n_step = kwargs.get(
            "n_step", self.n_input + self.n_output + self.n_sample - 2
        )
        ...
        self.time_aug_range = kwargs.get(
            "time_aug_range", [0.95, 0.975, 1.0, 1.025, 1.05]
        )
        ...
        if self.teacher_forcing:
            data = [
                x
                for x in data
                if len(x) > self.n_req + self.n_teach * self.n_step + self.buffer
            ]
        else:
            data = [x for x in data if len(x) > self.n_req + self.buffer]
        after_len = len(data)
        print(
            f"{original_len - after_len} samples were removed because they are too short. There are {after_len} remaining. \nIf this is not enough, try reducing n_step, n_teach and/or buffer and create a new instance of MappedDataAugGen"
        )

        self.aug_map = maps.augmentation_map(
            data,
            self.n_time,
            self.off_mode,
            lo_lim=self.lo_lim,
        ...
        self.__getitem__ = (
            self._tf_getitem if self.teacher_forcing else self._aug_getitem
        )
    ...
    def _tf_getitem(self, idx):
        ...
        if self.prev_map is None or map_["id"] != self.prev_map["id"]:
            ...
            sample = transpose(
                sample, map_["pitch_aug"], n_time=self.n_time, off_mode=self.off_mode
            )
            self.current_sample = sample[map_["strt_idx"] : map_["end_idx"]]

        sample = self.current_sample[
            map_["teach_idx"] : map_["teach_idx"] + self.n_req
        ].copy()
        ...
        for i in self.batch_loop:
            ...
            x[i, self.in_range, sub[: self.n_input]] = 1.0
            y[i, sub[self.n_input : self.n_io]] = 1.0
...
class MappedBalancedDataGen(Sequence):
    def __init__(self, data, **kwargs):
        ...
        self.map = maps.balanced_sampling_map(
            data, self.n_input, self.n_output, self.n_example, self.n_vocab
        )

        self.map = [
            self.map[x : x + self.n_sample]
            for x in range(0, len(self.map), self.n_sample)
        ]
    ...
    def __getitem__(self, idx):
        ...
        for i in self.batch_loop:
            sample = self.data[map_[i]["orig_sample_idx"]].copy()[
                map_[i]["strt_idx"] : map_[i]["end_idx"]
            ]
            x[i, self.in_range, sample[: self.n_input]] = 1.0
            y[i, sample[self.n_input : self.n_io]] = 1.0
