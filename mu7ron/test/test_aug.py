...
class TestAug(unittest.TestCase):
    def test_translate(self):
        ...
        for _ in range(100):
            ...
            for i in range(0, 64, 8):
                input_sequence = np.random.randint(
                    n_time + i, n_t_p - i, size=np.random.randint(1, 1000)
                )
                for j in range(-64, 64, 8):
                    result = aug.transpose(
                        input_sequence, j, n_time=n_time, n_t_p=n_t_p, off_mode=off_mode
                    )
        ...
        result = aug.transpose(
            t, -7, n_time=p["n_time"], n_t_p=n_t_p, off_mode=p["off_mode"]
        )
        self.assertTrue(np.array_equal(v["t0"], result))
        result = aug.transpose(
            t, 13, n_time=p["n_time"], n_t_p=n_t_p, off_mode=p["off_mode"]
        )
